 ...
 * (FR-024, `POST /api/v1/auth/login`). `setToken` también queda disponible
 * para cualquier otro mecanismo de obtención del token.
 *
 * El estado vive en memoria (signal) e hidrata desde `localStorage` al
 * arrancar la app, para sobrevivir a recargas de página. La verificación
 * real de validez del token (firma, expiración estricta) siempre la hace el
 * backend; aquí solo se usa para decisiones de UX (Principio II).
 */
...
export class AuthService {
  ...
  /**
   * `POST /api/v1/auth/login` (FR-024). Guarda automáticamente el token
   * recibido en caso de éxito. El backend es la autoridad final sobre la
   * validez de las credenciales (401 en caso contrario).
   */
