...
- [X] T003 Crear el workflow reutilizable `.github/workflows/_constitution-guardian.yml` (`workflow_call`) que invoca la API del agente contra `.specify/pipeline-constitution.md` (y `.specify/memory/constitution.md` cuando la PR toca código de aplicación) — FR-004, ADR-P4
...
- [X] T048 [P] Crear `.github/workflows/pipeline-config-guardian.yml`, trigger `pull_request` con `paths: ['.github/workflows/**', '.specify/pipeline-constitution.md', 'pipeline-spec.md']`, job `constitution-guardian` reutilizando el workflow de T003 (FR-020) — usa la ruta real `specs/002-cicd-pipeline-branching/**` en vez del nombre genérico `pipeline-spec.md`; ver T056 (2026-07-15), que crea ese archivo como puntero
...
- [X] T056 Crear `pipeline-spec.md` (raíz) como puntero al contenido real en `specs/002-cicd-pipeline-branching/` — resuelve la discrepancia de nombre literal que pide el enunciado del reto sin duplicar contenido; añadido `pipeline-spec.md` al filtro `paths:` de `pipeline-config-guardian.yml` (T048) junto a la ruta real, para que un cambio en cualquiera de las dos dispare el guardián (2026-07-15)
- [X] T057 Mover `pipeline-constitution.md` de la raíz a `.specify/pipeline-constitution.md`, siguiendo la estructura sugerida por el enunciado del reto (`/.specify/` o `/specs/`); actualizado el filtro `paths:` de `pipeline-config-guardian.yml`, el prompt de `_constitution-guardian.yml`, el link en `pipeline-spec.md` y las referencias en `plan.md`/`research.md`/`spec.md`/README (2026-07-15)
